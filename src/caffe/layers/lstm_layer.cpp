...
void LstmLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  gates_buffer_.Reshape(num_, 4 * channels_, 1, 1);
...
void LstmLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < num_; ++n) {
    for (int i = 0; i < channels_; ++i) {
      ...
      next_memory_state[idx] = prev_state_data[idx] * sigmoid(forget_gates[idx]) +
          sigmoid(input_gates[idx]) * tanh(input_values[idx]);
      next_hidden_state[idx] = next_memory_state[idx] *
  ...
  //     }
  //     CHECK_NEAR(forget_gate_mult,
  //         forget_gates[state_offset + i],
  //         Dtype(0.01));
  //     input = tanh(input);
  ...
  //     CHECK_NEAR(next_memory_state[state_offset + i],
  //         prev_state_data[state_offset + i] * forget_gate_mult + input * input_gate_mult,
  //         Dtype(0.01));
  //     next_memory_state[state_offset + i] = prev_state_data[state_offset + i] * forget_gate_mult + input * input_gate_mult;
  //     CHECK_NEAR(next_hidden_state[state_offset + i],
  //                next_memory_state[state_offset + i] * output_gate_mult,
  //                Dtype(0.01));
  //     next_hidden_state[state_offset + i] = next_memory_state[state_offset + i] * output_gate_mult;
  //   }
  // }
}
